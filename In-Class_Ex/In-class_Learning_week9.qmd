---
title: "In-class_Learning_week09"
author: "HTC"
editor: visual
...
```{r}
acc_Hansen_0.5 <- data.frame(SpatialAcc::ac(hexagons$demand,
                            eldercare$capacity,
                            distmat_km, 
                            #d0 = 50,
                            power = 0.5, 
...
```{r}
acc_Hansen_1 <- data.frame(SpatialAcc::ac(hexagons$demand,
                            eldercare$capacity,
                            distmat_km, 
                            d0 = 50,
                            power = 1, 
...
```{r}
acc_Hansen_1.5 <- data.frame(SpatialAcc::ac(hexagons$demand,
                            eldercare$capacity,
                            distmat_km, 
                            d0 = 50,
                            power = 1.5, 
...
```{r}
acc_Hansen_2 <- data.frame(SpatialAcc::ac(hexagons$demand,
                            eldercare$capacity,
                            distmat_km, 
                            d0 = 50,
                            power = 2, 
...
```{r}
acc_Hansen_2.5 <- data.frame(SpatialAcc::ac(hexagons$demand,
                            eldercare$capacity,
                            distmat_km, 
                            d0 = 50,
                            power = 2.5, 
...
```{r}
hexagons$accHansen_0.5 <- acc_Hansen_0.5$Hansen
hexagons$accHansen_1   <- acc_Hansen_1$Hansen
hexagons$accHansen_1.5 <- acc_Hansen_1.5$Hansen
hexagons$accHansen_2   <- acc_Hansen_2$Hansen
hexagons$accHansen_2.5 <- acc_Hansen_2.5$Hansen

names(hexagons)
```

```{r}
colnames(acc_Hansen_0.5) <- "acc_Hansen_0.5"
acc_SAM <- as_tibble(acc_Hansen_0.5)
hexagon_0.5 <- bind_cols(hexagons, acc_Hansen_0.5)
```

```{r}
colnames(acc_Hansen_1) <- "acc_Hansen_1"
acc_SAM <- as_tibble(acc_Hansen_1)
hexagon_1 <- bind_cols(hexagons, acc_Hansen_1)
```

```{r}
colnames(acc_Hansen_1.5) <- "acc_Hansen_1.5"
acc_SAM <- as_tibble(acc_Hansen_1.5)
hexagon_1.5 <- bind_cols(hexagons, acc_Hansen_1.5)
```

```{r}
colnames(acc_Hansen_2) <- "acc_Hansen_2"
acc_SAM <- as_tibble(acc_Hansen_2)
hexagon_2 <- bind_cols(hexagons, acc_Hansen_2)
```

```{r}
colnames(acc_Hansen_2.5) <- "acc_Hansen_2.5"
acc_SAM <- as_tibble(acc_Hansen_2.5)
hexagon_2.5 <- bind_cols(hexagons, acc_Hansen_2.5)
```

```{r}
tm_shape(hexagon_0.5,
         bbox = mapex) + 
  tm_polygons(fill = "acc_Hansen_0.5", 
...
```{r}
tm_shape(hexagon_1.5,
         bbox = mapex) + 
  tm_polygons(fill = "acc_Hansen_1.5", 
              fill.scale = tm_scale_intervals(
                style = "quantile",
                n = 10,
                values = "brewer.blues")) +
...
```{r}
tm_shape(hexagon_2.5,
         bbox = mapex) + 
  tm_polygons(fill = "acc_Hansen_2.5", 
              fill.scale = tm_scale_intervals(
                style = "quantile",
                n = 10,
                values = "brewer.blues")) +
