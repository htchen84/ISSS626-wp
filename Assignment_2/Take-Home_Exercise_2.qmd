...
hex_in_subzone \<- st_intersection( mpsz %\>% select(SUBZONE_N), hex_ok %\>% select(TOTAL_TRIPS, period) )

subzone_trips \<- hex_in_subzone %\>% st_drop_geometry() %\>% group_by(SUBZONE_N, period) %\>% summarise(TOTAL_TRIPS = sum(TOTAL_TRIPS, na.rm = TRUE), .groups = "drop")

mpsz_trips \<- mpsz %\>% left_join(subzone_trips, by = "SUBZONE_N")
...
tm_shape(mpsz_trips) +
  ...
  tm_layout(
    main.title = "Passenger Trips by Subzone (Study Area Count Style)",
    legend.outside = TRUE
  )
...
-   **Spatial distribution pattern** —\
    The presence of large red patches across the city suggests that trip volumes are not randomly distributed, but rather spatially structured, with clear regional clustering (high ridership in the east/central, lower in west/industrial zones).
...
```         
**Hot spots (red)** form around:

The north (Woodlands–Yishun corridor)

The west (Jurong West and Tuas industrial belt)

The east (Tampines–Pasir Ris region)\
These are major residential and transit hubs, indicating zones of concentrated trip activity and strong public-transport demand.

**Cold spots (blue)** dominate:

Peripheral or less populated areas such as Lim Chu Kang, western coastlines, and Changi Coast\
These reflect zones of low trip density, likely industrial or restricted-access regions with minimal commuter flow.

**Central areas (grey)** show insignificant clustering — trip patterns there are mixed or transitional, with both high- and low-density hexagons interspersed.
```
...
```{r}
#c
mpsz_trips_complete <- mpsz_trips %>%
  tidyr::expand(HEX_ID, bus_tap_on_time) %>%     # all HEX–time combinations
  left_join(mpsz_trips_clean, by = c("HEX_ID", "bus_tap_on_time")) %>%
  mutate(TOTAL_TRIPS = replace_na(TOTAL_TRIPS, 0)) %>%
  # bring geometry from hexagon (spatial grid)
...
```{r}
# Convert to an ordered factor (morning < afternoon < weekend)
mpsz_trips_complete <- mpsz_trips_complete %>%
  mutate(
